...
class c_pcategory extends SAdmin{
	...
	//按cat_id分类ID删除产品, 此分类已删除无需更新数量
	private function DeleteProductByCategoryId($cat_id) {
		$products = $this->db->query("SELECT pro_id, cat_id, path, filename FROM " . TABLE_PREFIX . "product WHERE cat_id = '$cat_id'");

		if(empty($products)) return;

		$this->db->exe("DELETE FROM " . TABLE_PREFIX . "product WHERE cat_id = '$cat_id'");//删除所有产品

		while($product = $this->db->fetch($products)){
			$this->UnlinkImage($product['path'], $product['filename']); //删除主图片
			$this->DeleteGroupImage($product['pro_id']); //删除组图片
		}
	}

	//按产品pro_id 或组图片g_id 删除其图片
	private function DeleteGroupImage($pro_id) {
		$getimages = $this->db->query("SELECT g_id, path, filename FROM " . TABLE_PREFIX . "gimage WHERE pro_id = '$pro_id'");
	...
	public function delete(){
		...
		if(IsPost('confirmdelete'))	{
			if($p_id){
				$getcounts = $this->db->getOne("SELECT counts FROM " . TABLE_PREFIX . "pcat WHERE cat_id = '$cat_id'");
				$this->db->exe("DELETE FROM " . TABLE_PREFIX . "pcat WHERE cat_id = '$cat_id'");
				$this->db->exe("UPDATE " . TABLE_PREFIX . "product SET cat_id = '$p_id' WHERE cat_id = '$cat_id' ");
				$this->db->exe("UPDATE " . TABLE_PREFIX . "pcat SET counts = (counts +$getcounts[counts]) WHERE cat_id = '$p_id'");
			}else{
				$this->db->exe("DELETE FROM " . TABLE_PREFIX . "pcat WHERE cat_id = '$cat_id'");
				$this->DeleteProductByCategoryId($cat_id);	//删除分类的所有产品
			}

			Success('pcategory');

		}else{
			$parent_cat = $this->db->getOne("SELECT cat_id  FROM " . TABLE_PREFIX . "pcat WHERE p_id = '$cat_id'");
			if($parent_cat) {
				Error('当前分类有下级分类, 请先删除其下级分类!', '删除分类错误');
			}

			SubMenu('产品分类管理', array(array('添加分类', 'pcategory/add'),array('删除分类', 'pcategory/delete?cat_id='.$cat_id . '&cats=' . $cats, 1),array('全部分类', 'pcategory'), array('产品列表', 'products')));

			$category = $this->db->getOne("SELECT cat_id, p_id, name  FROM " . TABLE_PREFIX . "pcat WHERE cat_id = '$cat_id'");


			echo '<form method="post" action="'.BURL('pcategory/delete').'">
			...
			Iif($cats > 1, '<span class=note>此分类下所有的产品将自动转入</span>: ' . $this->GetCategorySelect($cat_id, $category['p_id'], 0), '<span class=note>注: 此分类下所有的产品将被删除!</span>') . '<BR><BR>', '<input type="submit" name="confirmdelete" value="确定删除" class="save">
	...
	public function save(){
		...
		if(isset($errors)){
			...
			if($cat_id){
				$this->db->exe("UPDATE " . TABLE_PREFIX . "pcat SET 
				p_id= '$p_id',
				sort= '$sort',
				is_show= '$is_show',
				show_sub= '$show_sub',
				...
				$this->db->exe("INSERT INTO " . TABLE_PREFIX . "pcat (p_id, is_show, show_sub, name, name_en, keywords, keywords_en, desc_cn, desc_en) VALUES ('$p_id', '$is_show', '$show_sub', '$name', '$name_en', '$keywords', '$keywords_en', '$desc_cn', '$desc_en') ");

				$cat_id = $this->db->insert_id;
				$this->db->exe("UPDATE " . TABLE_PREFIX . "pcat SET sort = '$cat_id' WHERE cat_id = '$cat_id'");
			}
	...
	public function updatecategories(){
		...
		for($i = 0; $i < count($cat_ids); $i++){
			$this->db->exe("UPDATE " . TABLE_PREFIX . "pcat SET sort = '". ForceInt($sorts[$i])."',
	...
	public function add(){
		$cat_id = ForceIntFrom('cat_id');
		if($cat_id){
			SubMenu('产品分类管理', array(array('添加分类', 'pcategory/add'),array('编辑分类', 'pcategory/add?cat_id='.$cat_id, 1),array('全部分类', 'pcategory'), array('产品列表', 'products')));

			$category = $this->db->getOne("SELECT * FROM " . TABLE_PREFIX . "pcat WHERE cat_id = '$cat_id'");
		}else{
			SubMenu('产品分类管理', array(array('添加分类', 'pcategory/add', 1),array('全部分类', 'pcategory'), array('产品列表', 'products')));
		...
		<form id="editorform88" name="editorform88" method="post" action="'.BURL('pcategory/save').'">
		<input type="hidden" name="cat_id" value="' . $cat_id . '" />';
		...
		TableRow(array('<B>是否发布?</B>', '<input type="checkbox" name="is_show" value="1" '.Iif($category['is_show'] == 1, 'CHECKED').'> <b>是:</b> <span class=light>当不发布时, 此分类及其下属分类, 以及这些分类下的所有产品均不显示.</span>'));

		TableRow(array('<B>是否显示下级?</B>', '<input type="checkbox" name="show_sub" value="1" '.Iif($category['show_sub'], 'CHECKED').'> <b>是:</b> <span class=light>打开此分类时, 是否同时显示所有下级分类的产品?</span>'));
	...
	public function index(){
		SubMenu('产品分类管理', array(array('添加分类', 'pcategory/add'),array('产品列表', 'products')));

		$getcategories = $this->db->query("SELECT cat_id, p_id, sort, is_show, show_sub, name, name_en, counts FROM " . TABLE_PREFIX . "pcat ORDER BY sort");
		$this->cats = $this->db->result_nums;

		echo '<form method="post" action="'.BURL('pcategory/updatecategories').'">';

		TableHeader('产品分类列表('.$this->cats.'个)');
		TableRow(array('排序编号 与 分类名称', '产品数', '状态', '显示下级分类', '编辑', '删除'), 'tr0');

		if($this->cats < 1){
			TableRow('<center><BR><font class=redb>暂无任何产品分类!</font><BR><BR></center>');
		}else{
	...
	private function ShowCategories ($parentid = 0, $sublevelmarker = '') {
		...
		foreach($allcategories as $key => $value){
			if($parentid == $value){
				...
				'<a href="' . BURL('pcategory/edit?cat_id=' . $key) . '"><img src="' . SYSDIR . 'public/admin/images/edit.png" /></a>',
				'<a href="' . BURL('pcategory/delete?cat_id=' . $key . '&cats=' . $this->cats) . '"><img src="' . SYSDIR . 'public/admin/images/trash.png" /></a>'));
